classdef salsunFinalRotation1dLayer < nnet.layer.Layer %#codegen
    %SALSUNFINALROTATION1DLAYER
    %
    ...
    methods
        function layer = salsunFinalRotation1dLayer(varargin)
        ...
        function Z = predict(layer, X, Theta)
            ...
            if isgpuarray(X)
                ...
                if nAnglesH > 0
                    ...
                    W0T = permute(fcn_orthmtxgenW(anglesW_,muW_),[2 1 3]);
                    U0T = permute(fcn_orthmtxgenU(anglesU_,muU_),[2 1 3]);
                else
                    W0T = reshape(muW_,pt,1,[]);
                    U0T = reshape(muU_,pb,1,[]);
                end
                Zt = reshape(pagefun(@mtimes,W0T,Yt_ext),pt,1,nblks,nSamples);
                Zb = reshape(pagefun(@mtimes,U0T,Yb_ext),pb,1,nblks,nSamples);
            else
                ...
                for iSample = 1:nSamples
                    ...
                    if nAnglesH > 0
                        ...
                        W0T_i = permute(fcn_orthmtxgenW(anglesW,muW),[2 1 3]);
                        U0T_i = permute(fcn_orthmtxgenU(anglesU,muU),[2 1 3]);
                    else
                        W0T_i = reshape(muW,pt,1,[]);
                        U0T_i = reshape(muU,pb,1,[]);
                    end
                    for iblk = 1:nblks
                        Zt(:,:,iblk,iSample) = W0T_i(:,:,iblk)*Yt(:,:,iblk,iSample);
                        Zb(:,:,iblk,iSample) = U0T_i(:,:,iblk)*Yb(:,:,iblk,iSample);
                    end
                end
            end
            Z = cat(1,Zt,Zb);
        ...
        function [dLdX, dLdTheta] = backward(layer, X, Theta, ~, dLdZ, ~)
            ...
            if isgpuarray(X)
                ...
                if nAnglesH > 0
                    ...
                    dLdX(1:pt,:,:,:) = reshape(pagefun(@mtimes,W0,cdLd_top_ext),pt,1,nblks,nSamples);
                    dLdX(pt+1:pt+pb,:,:,:) = reshape(pagefun(@mtimes,U0,cdLd_btm_ext),pb,1,nblks,nSamples);

                    % dLdTheta_i = <dLdZ,(dVdTheta_i)X> (transposed
                    % per-angle derivative, matching predict), batched
                    % across blocks and samples together.
                    ...
                    for iAngle = uint32(1:nAnglesH)
                        ...
                        dWT = permute(dW,[2 1 3]);
                        dUT = permute(dU,[2 1 3]);
                        d_top_ext = pagefun(@mtimes,dWT,c_top_ext);
                        d_btm_ext = pagefun(@mtimes,dUT,c_btm_ext);
                ...
                for iSample = 1:nSamples
                    ...
                    if nAnglesH > 0
                        ...
                        for iblk = 1:nblks
                            dLdX(1:pt,:,iblk,iSample) = W0_i(:,:,iblk)*dLdZ(1:pt,:,iblk,iSample);
                            dLdX(pt+1:pt+pb,:,iblk,iSample) = U0_i(:,:,iblk)*dLdZ(pt+1:pt+pb,:,iblk,iSample);
                        end

                        % dLdTheta_i = <dLdZ,(dVdTheta_i)X> (transposed
                        % per-angle derivative, matching predict).
                        ...
                        for iAngle = uint32(1:nAnglesH)
                            ...
                            dUT = permute(dU,[2 1 3]);
                            for iblk = 1:nblks
                                d_top = dWT(:,:,iblk)*X(1:pt,:,iblk,iSample);
                                d_btm = dUT(:,:,iblk)*X(pt+1:pt+pb,:,iblk,iSample);
