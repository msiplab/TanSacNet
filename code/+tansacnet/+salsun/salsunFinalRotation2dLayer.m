classdef salsunFinalRotation2dLayer < nnet.layer.Layer %#codegen
    ...
    methods
        ...
        function Z = predict(layer, X, Theta)
            ...
            if isgpuarray(X)
                ...
                W0T = permute(fcn_orthmtxgen(anglesW_,muW_),[2 1 3]);
                U0T = permute(fcn_orthmtxgen(anglesU_,muU_),[2 1 3]);
                Ys_ext = reshape(Y(1:ps,:,:),ps,1,nrows*ncols*nSamples);
                Ya_ext = reshape(Y(ps+1:ps+pa,:,:),pa,1,nrows*ncols*nSamples);
                Zs = reshape(pagefun(@mtimes,W0T,Ys_ext),ps,nrows*ncols,nSamples);
                Za = reshape(pagefun(@mtimes,U0T,Ya_ext),pa,nrows*ncols,nSamples);
            else
                for iSample = 1:nSamples
                    ...
                    for iblk = 1:nrows*ncols
                        Zs(:,iblk,iSample) = W0T_i(:,:,iblk)*Y(1:ps,iblk,iSample);
                        Za(:,iblk,iSample) = U0T_i(:,:,iblk)*Y(ps+1:ps+pa,iblk,iSample);
                    end
                end
            end
        ...
        function [dLdX, dLdTheta] = backward(layer, X, Theta, ~, dLdZ, ~)
            ...
            if isgpuarray(dLdZ)
                ...
                Zs = reshape(pagefun(@mtimes,W0,dldz_upp_ext),ps,nrows*ncols,nSamples);
                Za = reshape(pagefun(@mtimes,U0,dldz_low_ext),pa,nrows*ncols,nSamples);

                % dLdWi = <dLdZ,(dVdWi)X> (Mode='Synthesis' selects the
                % transposed per-angle derivative, matching predict),
                % batched across blocks and samples together.
                ...
                for iAngle = uint32(1:nAngles)
                    ...
                    d_upp = reshape(pagefun(@mtimes,dWT,c_upp_ext),ps,nrows*ncols,nSamples);
                    d_low = reshape(pagefun(@mtimes,dUT,c_low_ext),pa,nrows*ncols,nSamples);
                ...
                for iSample = 1:nSamples
                    ...
                    for iblk = 1:nrows*ncols
                        Zs(:,iblk,iSample) = W0_i(:,:,iblk)*dldz_upp(:,iblk,iSample);
                        Za(:,iblk,iSample) = U0_i(:,:,iblk)*dldz_low(:,iblk,iSample);
                    end
                    ...
                    for iAngle = uint32(1:nAngles)
                        ...
                        dUT = permute(dU,[2 1 3]);
                        for iblk = 1:nrows*ncols
                            d_upp = dWT(:,:,iblk)*c_upp(:,iblk,iSample);
                            d_low = dUT(:,:,iblk)*c_low(:,iblk,iSample);
