classdef lsunBlockIdct1dLayerTestCase < matlab.unittest.TestCase
    ...
    methods (Test)
        ...
        function testPredict(testCase, ...
            ...
            nblks = seqlen/stride;
            %nComponents = 1;
            X = rand(stride,1,nblks,nSamples,datatype);
            
            % Expected values
            expctdZ = zeros(1,seqlen,1,nSamples,datatype);
        ...
        function testBackward(testCase, ...
            ...
            nSamples = 8;
            %nComponents = 1;
            dLdZ = rand(1, seqlen, 1, nSamples, datatype);
            
            % Expected values
            nblks = seqlen/stride;
            expctddLdX = zeros(stride,1,nblks,nSamples,datatype);
            for iSample = 1:nSamples
                % Block DCT
