classdef lsunInitialRotation2dLayerTestCase < matlab.unittest.TestCase
    ...
    function testBackwardGrayscaleWithDeviceAndDType(testCase, ...
            ...
            U0T = permute(genU.step(anglesU,mus_,0),[2 1 3]);
            Y = dLdZ; % permute(dLdZ,[3 1 2 4]);
            Ys = reshape(Y(1:ps,:,:,:),ps,nrows_*ncols_,nSamples);
            Ya = reshape(Y(ps+1:ps+pa,:,:,:),pa,nrows_*ncols_,nSamples);
            for iSample = 1:nSamples
                for iblk = 1:(nrows_*ncols_)
                    Ys(:,iblk,iSample) = W0T(1:ps,:,iblk)*Ys(:,iblk,iSample); 
                    Ya(:,iblk,iSample) = U0T(1:pa,:,iblk)*Ya(:,iblk,iSample);
            ...
            %expctddLdX = ipermute(reshape(Zsa,nDecs,nrows,ncols,nSamples),...
            %    [3 1 2 4]);
            expctddLdX = reshape(Zsa,nDecs,nrows_,ncols_,nSamples);
            
            % dLdWi = <dLdZ,(dVdWi)X>
            dldw_ = zeros(2*nAnglesH,nrows_*ncols_,1,datatype);
            dldz_ = dLdZ; %permute(dLdZ,[3 1 2 4]);
            dldz_upp = reshape(dldz_(1:ps,:,:,:),ps,nrows_*ncols_,nSamples);
            dldz_low = reshape(dldz_(ps+1:ps+pa,:,:,:),pa,nrows_*ncols_,nSamples);
            % (dVdWi)X
            a_ = X; %permute(X,[3 1 2 4]);
            c_upp = reshape(a_(1:ps,:,:,:),ps,nrows_*ncols_,nSamples);
            c_low = reshape(a_(ps+1:nDecs,:,:,:),pa,nrows_*ncols_,nSamples);
            for iAngle = 1:nAnglesH
                dW0 = genW.step(anglesW,mus_,iAngle);
                dU0 = genU.step(anglesU,mus_,iAngle);
                for iblk = 1:(nrows_*ncols_)                
        ...
        function testBackwardGrayscaleWithRandomAnglesNoDcLeackage(testCase, ...
            ...
            musW = mus*ones(ps,nrows*ncols,datatype);
            musW(1,:) = ones(1,nrows*ncols,datatype);
            musU = mus*ones(pa,nrows*ncols,datatype);            
