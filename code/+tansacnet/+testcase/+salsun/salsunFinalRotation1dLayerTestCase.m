classdef salsunFinalRotation1dLayerTestCase < matlab.unittest.TestCase
    %SALSUNFINALROTATION1DLAYERTESTCASE
    %
    ...
    methods (TestClassTeardown)
        ...
        function finalCheck(~)
            ...
            nBlks = 8;
            layer = salsunFinalRotation1dLayer(...
                'Stride',stride_,...
    ...
    methods (Test)
        ...
        function testPredict(testCase, ...
            ...
            W0T = repmat(eye(pt,datatype),[1 1 nblks nSamples]);
            U0T = repmat(eye(pb,datatype),[1 1 nblks nSamples]);
            ...
            for iSample = 1:nSamples
                ...
                U0Ti = U0T(:,:,:,iSample);
                for iblk = 1:nblks
                    Yi(1:pt,:,iblk) = W0Ti(:,:,iblk)*Yi(1:pt,:,iblk);
                    Yi(pt+1:pt+pb,:,iblk) = U0Ti(:,:,iblk)*Yi(pt+1:pt+pb,:,iblk);
                end
                expctdZ(:,:,:,iSample) = Yi;
            end
        ...
        function testPredictWithDeviceAndDType(testCase, ...
            ...
            pt = ceil(nChsTotal/2);
            pb = floor(nChsTotal/2);
            W0T = repmat(eye(pt,datatype),[1 1 nblks_ nSamples]);
            U0T = repmat(eye(pb,datatype),[1 1 nblks_ nSamples]);
        ...
        function testPredictWithRandomAnglesDataTypeAndDevice(testCase, ...
            ...
            for iSample = 1:nSamples
                ...
                if nAnglesH == 0
                    ...
                    W0T = permute(genW.step(anglesW,mus_),[2 1 3]);
                    U0T = permute(genU.step(anglesU,mus_),[2 1 3]);
                end
                Yi = X(:,:,:,iSample);
                for iblk = 1:nblks
        ...
        function testBackwardWithZeroAngles(testCase, ...
            ...
            % Expected values: dLdX uses the UNTRANSPOSED W0/U0
            % (Final's predict already applied the transpose).
            if nAnglesH == 0
            ...
            for iAngle = 1:nAnglesH
                dWT = permute(genW.step(anglesW,mus_,iAngle),[2 1 3]);
                dUT = permute(genU.step(anglesU,mus_,iAngle),[2 1 3]);
                for iblk = 1:nblks
                    for iSample = 1:nSamples
        ...
        function testBackwardWithRandomAnglesDataTypeAndDevice(testCase, ...
            ...
            for iSample = 1:nSamples
                ...
                dLdZi = dLdZ(:,:,:,iSample);
                for iblk = 1:nblks
                    dLdZi(1:pt,:,iblk) = W0(:,:,iblk)*dLdZi(1:pt,:,iblk);
                    dLdZi(pt+1:pt+pb,:,iblk) = U0(:,:,iblk)*dLdZi(pt+1:pt+pb,:,iblk);
                end
                ...
                for iAngle = 1:nAnglesH
                    ...
                    dUT = permute(genU.step(anglesU,mus_,iAngle),[2 1 3]);
                    for iblk = 1:nblks
                        d_top = dWT(:,:,iblk)*Xi(1:pt,:,iblk);
                        d_btm = dUT(:,:,iblk)*Xi(pt+1:pt+pb,:,iblk);
