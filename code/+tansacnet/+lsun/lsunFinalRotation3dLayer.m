classdef lsunFinalRotation3dLayer < nnet.layer.Layer %#codegen
    %LSUNFINALROTATION3DLAYER
    %
    ...
    methods
        function layer = lsunFinalRotation3dLayer(varargin)
        ...
        function Z = predict(layer, X)
            ...
            %    [4 1 2 3 5]);
            Z = reshape(Zsa,ps+pa,nrows,ncols,nlays,nSamples);
        ...
        function [dLdX, dLdW] = backward(layer, X, ~, dLdZ, ~)
            ...
            dW0Pre = repmat(eye(ps,'like',W0),[1 1 (nrows*ncols*nlays)]);
            dU0Pre = repmat(eye(pa,'like',U0),[1 1 (nrows*ncols*nlays)]);
            
            % Layer backward function goes here.
            % dLdX = dZdX x dLdZ
            %adldz_ = dLdZ; %permute(dLdZ,[4 1 2 3 5]);
            %cdLd_ = reshape(adldz_,nDecs,nrows*ncols*nlays*nSamples);
            cdLd_ = reshape(dLdZ,ps+pa,nrows*ncols*nlays,nSamples);
            cdLd_upp = zeros(ps,nrows*ncols*nlays,nSamples,'like',cdLd_);
            cdLd_low = zeros(pa,nrows*ncols*nlays,nSamples,'like',cdLd_);
            for iSample = 1:nSamples
                if isgpuarray(X)
            ...
            %adLd_ = reshape(cat(1,cdLd_upp,cdLd_low),pa+ps,nrows,ncols,nlays,nSamples);
            %dLdX = adLd_; %ipermute(adLd_,[4 1 2 3 5]);
            dLdX = reshape(cat(1,cdLd_upp,cdLd_low),pa+ps,nrows,ncols,nlays,nSamples);
