classdef lsunInitialRotation1dLayer < nnet.layer.Layer %#codegen
    %LSUNINITIALROTATION1DLAYER
    %
    ...
    methods
        ...
        function layer = lsunInitialRotation1dLayer(varargin)
        ...
        function Z = predict(layer, X)
            ...
            Yb = X(pt+1:pt+pb,:,:,:);
            if isgpuarray(X)
                Zt = pagefun(@mtimes,W0_,Yt);
                Zb = pagefun(@mtimes,U0_,Yb);
            else
                Zt = zeros(pt,1,nblks,nSamples,'like',X);
                Zb = zeros(pb,1,nblks,nSamples,'like',X);
                for iSample = 1:nSamples
                    for iblk = 1:nblks
                        Zt(:,:,iblk,iSample) = W0_(:,:,iblk)*Yt(:,:,iblk,iSample);
                        Zb(:,:,iblk,iSample) = U0_(:,:,iblk)*Yb(:,:,iblk,iSample);
                    end
                end
            end
            Z = cat(1,Zt,Zb);
        ...
        function [dLdX, dLdW] = backward(layer, X, ~, dLdZ, ~)
            ...
            dLdX = dLdZ;
            if isgpuarray(X)
                dLdX(1:pt,:,:,:) = pagefun(@mtimes,W0T,dLdZ(1:pt,:,:,:));
                dLdX(pt+1:pt+pb,:,:,:) = pagefun(@mtimes,U0T,dLdZ(pt+1:pt+pb,:,:,:));
            else
                for iSample = 1:nSamples
                    for iblk = 1:nblks
                        dLdX(1:pt,:,iblk,iSample) = W0T(:,:,iblk)*dLdZ(1:pt,:,iblk,iSample);
                        dLdX(pt+1:pt+pb,:,iblk,iSample) = U0T(:,:,iblk)*dLdZ(pt+1:pt+pb,:,iblk,iSample);
                    end
                end
            end
            ...
            for iAngle = uint32(1:nAngles/2)
                ...
                if isgpuarray(X)
                    d_top = pagefun(@mtimes,dW0,c_top);
                    d_btm = pagefun(@mtimes,dU0,c_btm);
                    ...
                    for iblk = 1:nblks
                        ...
                        for iSample = 1:nSamples
                            d_top_iblk(:,iSample) = dW0(:,:,iblk)*c_top_iblk(:,iSample);
                            d_btm_iblk(:,iSample) = dU0(:,:,iblk)*c_btm_iblk(:,iSample);
                        end
        ...
        function layer = updateParameters(layer)
            ...
            if nAngles > 0
                ...
                layer.W0 = reshape(muW,1,1,[]);
                layer.U0 = reshape(muU,1,1,[]);
            end
