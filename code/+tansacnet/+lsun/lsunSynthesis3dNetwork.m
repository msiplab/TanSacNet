classdef lsunSynthesis3dNetwork < dlnetwork
    %LSUNSYNTHESIS3dNETWORK
    %
    ...
    methods
        function obj = lsunSynthesis3dNetwork(varargin)
            % See the property settings for fcn_createlsunlgraph2d
            p = inputParser;
            addParameter(p,'InputSize',[32 32 32]);
            addParameter(p,'NumberOfComponents',1);
            addParameter(p,'NumberOfVanishingMoments',[1 1 1]);
            addParameter(p,'Stride',[2 2 2]);
            addParameter(p,'OverlappingFactor',[1 1 1]);
    ...
    methods (Hidden=false)
        function dlnet = dlnetwork(obj)
            % See the property settings for fcn_createlsunlgraph3d
            dlnet = [];
            ...
            if all(mod(stride,2))
                throw(MException('LsunLayer:InvalidStride',...
                    '[%d %d %d] : The product of stride should be even.',...
                    stride(1),stride(2),stride(3)))
            end
            if ~all(mod(ovlpFactor,2))
                throw(MException('LsunLayer:InvalidOverlappingFactor',...
                    '[%d %d %d] : Currently, odd overlapping factors are only supported.',...
                    ovlpFactor(1),ovlpFactor(2),ovlpFactor(3)))
            end

            nBlocks = inputSize(1:3)./stride;
            ...
            for iLv = 1:nLevels
                ...
                % Initial blocks
                % blockDctLayers{iLv} = lsunBlockDct3dLayer('Name',[prefix strLv 'E0'],...
                %     'Stride',stride,...
                ...
                %         lsunInitialRotation3dLayer('Name',[prefix strLv strCmp 'V0'],...
                ...
                % Final blocks
                blockIdctLayers{iLv} = lsunBlockIdct3dLayer('Name',[prefix strLv 'E0~'],...
                    'Stride',stride,...
                    'NumberOfComponents',nComponents);
                for iCmp = 1:nComponents
                    strCmp = sprintf('Cmp%0d_',iCmp);
                    synthesisLayers{iLv,iCmp} = [ synthesisLayers{iLv,iCmp}
                        lsunFinalRotation3dLayer('Name',[prefix strLv strCmp 'V0~'],...
                        'NumberOfBlocks',nBlocks,'Stride',stride,...
                ...
                for iCmp = 1:nComponents
                    ...
                    for iOrderH = 2:2:ovlpFactor(2)-1
                        % analysisLayers{iLv,iCmp} = [ analysisLayers{iLv,iCmp}
                        %     lsunAtomExtension3dLayer('Name',[prefix strLv strCmp 'Qh' num2str(iOrderH-1) 'rd'],...
                        %     'Stride',stride,'Direction','Right','TargetChannels','Difference')
                        %     lsunIntermediateRotation3dLayer('Name',[prefix strLv strCmp 'Vh' num2str(iOrderH-1)],...
                        %     'Stride',stride,'NumberOfBlocks',nBlocks,'Mode','Analysis','Mus',-1,...
                        %     'Device',device,'DType',dtype)
                        %     lsunAtomExtension3dLayer('Name',[prefix strLv strCmp 'Qh' num2str(iOrderH) 'ls'],...
                        %     'Stride',stride,'Direction','Left','TargetChannels','Sum')
                        %     lsunIntermediateRotation3dLayer('Name',[prefix strLv strCmp 'Vh' num2str(iOrderH) ],...
                        %     'Stride',stride,'NumberOfBlocks',nBlocks,'Mode','Analysis','Mus',-1,...
                        %     'Device',device,...
                    ...
                    for iOrderV = 2:2:ovlpFactor(1)-1
                        % analysisLayers{iLv,iCmp} = [ analysisLayers{iLv,iCmp}
                        %     lsunAtomExtension3dLayer('Name',[prefix strLv strCmp 'Qv' num2str(iOrderV-1) 'dd'],...
                        %     'Stride',stride,'Direction','Down','TargetChannels','Difference')
                        %     lsunIntermediateRotation3dLayer('Name',[prefix strLv strCmp 'Vv' num2str(iOrderV-1)],...
                        %     'Stride',stride,'NumberOfBlocks',nBlocks,'Mode','Analysis','Mus',-1,...
                        %     'Device',device,...
                        %     'DType',dtype)
                        %     lsunAtomExtension3dLayer('Name',[prefix strLv strCmp 'Qv' num2str(iOrderV) 'us'],...
                        %     'Stride',stride,'Direction','Up','TargetChannels','Sum')
                        %     lsunIntermediateRotation3dLayer('Name',[prefix strLv strCmp 'Vv' num2str(iOrderV)],...
                        %     'Stride',stride,'NumberOfBlocks',nBlocks,'Mode','Analysis','Mus',-1,...
                        %     'Device',device,...
                        %     'DType',dtype) % Revised default Mus to -1 on 6 Sept. 2024
                        %     ];
                    ...
                    for iOrderD = 2:2:ovlpFactor(3)-1
                        % analysisLayers{iLv,iCmp} = [ analysisLayers{iLv,iCmp}
                        %     lsunAtomExtension3dLayer('Name',[prefix strLv strCmp 'Qd' num2str(iOrderD-1) 'bd'],...
                        %     'Stride',stride,'Direction','Back','TargetChannels','Difference')
                        %     lsunIntermediateRotation3dLayer('Name',[prefix strLv strCmp 'Vd' num2str(iOrderD-1)],...
                        %     'Stride',stride,'NumberOfBlocks',nBlocks,'Mode','Analysis','Mus',-1,...
                        %         'Device',device,...
                        %         'DType',dtype)
                        %     lsunAtomExtension3dLayer('Name',[prefix strLv strCmp 'Qd' num2str(iOrderD) 'fs'],...
                        %     'Stride',stride,'Direction','Front','TargetChannels','Sum')
                        %     lsunIntermediateRotation3dLayer('Name',[prefix strLv strCmp 'Vd' num2str(iOrderD)],...
                        %     'Stride',stride,'NumberOfBlocks',nBlocks,'Mode','Analysis','Mus',-1,...
                        %         'Device',device,...
                        %         'DType',dtype) % Revised default Mus to -1 on 6 Sept. 2024
                        %     ];
                        synthesisLayers{iLv,iCmp} = [ synthesisLayers{iLv,iCmp}
                            lsunAtomExtension3dLayer('Name',[prefix strLv strCmp 'Qd' num2str(iOrderD-1) 'bd~'],...
                            'Stride',stride,'Direction','Front','TargetChannels','Difference')
                            lsunIntermediateRotation3dLayer('Name',[prefix strLv strCmp 'Vd' num2str(iOrderD-1) '~'],...
                            'Stride',stride,'NumberOfBlocks',nBlocks,'Mode','Synthesis','Mus',-1,...
                                'Device',device,...
                                'DType',dtype)
                            lsunAtomExtension3dLayer('Name',[prefix strLv strCmp 'Qd' num2str(iOrderD) 'fs~'],...
                            'Stride',stride,'Direction','Back','TargetChannels','Sum')
                            lsunIntermediateRotation3dLayer('Name',[prefix strLv strCmp 'Vd' num2str(iOrderD) '~'],...
                            'Stride',stride,'NumberOfBlocks',nBlocks,'Mode','Synthesis','Mus',-1,...
                                'Device',device,...
                                'DType',dtype) % Revised default Mus to -1 on 6 Sept. 2024
                    ...
                    % analysisLayers{iLv,iCmp} = [ analysisLayers{iLv,iCmp}
                    %     lsunChannelSeparation3dLayer('Name',[prefix strLv strCmp 'Sp'])
                    %     ];


                    synthesisLayers{iLv,iCmp} = [ synthesisLayers{iLv,iCmp}
                        lsunChannelConcatenation3dLayer('Name',[prefix strLv strCmp 'Cn'])
                        ];
                end
            ...
            lsunSdlnet = lsunSdlnet.addLayers(...
                lsunComponentSeparation3dLayer(nComponents,'Name',[prefix strLv 'DcIn']));
            lsunSdlnet = lsunSdlnet.addLayers(...
                lsunComponentSeparation3dLayer(nComponents,'Name',[prefix strLv 'AcIn']));
            if nComponents > 1
                for iCmp = 1:nComponents
                    strCmp = sprintf('Cmp%0d_',iCmp);
            ...
            for iLv = 1:nLevels
                strLv = sprintf('Lv%0d_',iLv);
                inputSubSize(1:3) = inputSize(1:3)./(stride.^iLv);
                inputSubSize(4) = nComponents*(sum(nChannels)-1);
                if isapndinout
                    lsunSdlnet = lsunSdlnet.addLayers(...
                        image3dInputLayer(inputSubSize,...
            ...
            inputSubSize(1:3) = inputSize(1:3)./(stride.^nLevels);
            inputSubSize(4) = nComponents;
            if isapndinout
                lsunSdlnet = lsunSdlnet.addLayers(...
                    image3dInputLayer(inputSubSize,...
            ...
            % %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            % lsunSdlnet = addLayers(lsunSdlnet, image3dInputLayer(inputSize, ...
